...
- 10 份報告各保留 3 版：1 份 iPad／電腦共用的中文教學 HTML 加上中英文 GitHub Pages post，共 30 個交付檔。取消詳細版 HTML，不保留副本或導覽連結。
- 3 版都涵蓋該篇全部已核准 claim。中英文 post 的 claim、主軸、例子、限制與問題順序一致；HTML 可以採不同的漸進教學順序。
...
- 原先排除的主題，包括 Fabrics／NVMe-oF／Discovery／NQN，若是理解本篇所需背景，可以用最小篇幅解釋。須新增 `PREREQUISITE_ONLY` 項目，記下具體教學必要性、所支援主題及來源。此授權不自動納入整個排除章節或所有排除圖。
...
把 post 的折疊內容展開。中英文 post 保留報告正文與可展開的完整技術細節。
