...
COMMON = {
 ...
 'Dword': b('Dword（Double word）；32 bits，也就是 4 bytes。對比 word=16 bits；例如 zero-based dword count=3 代表 4 個 Dwords，也就是 16 bytes。','Dword (double word): 32 bits, or 4 bytes. A word is 16 bits; for example, a zero-based dword count of 3 represents 4 Dwords, or 16 bytes.'),
 ...
 'bit range': b('bit range；欄位占用的位元範圍，例如 bits 7:4 共有 4 bits；不能把 bit 編號直接當成 byte 數。','bit range: the bits occupied by a field, such as bits 7:4 being 4 bits; bit numbers are not byte counts.'),
 'raw value': b('raw value；欄位直接讀出的數值，尚未套用 zero-based、單位或縮放規則；先確認定義再換算。','raw value: the value read directly from a field before applying zero-based, unit, or scaling rules; confirm the definition before converting it.'),
