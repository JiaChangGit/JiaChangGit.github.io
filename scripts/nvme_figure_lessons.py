...
def lesson(figure):
    if figure['report_id'] == 'base-endurance-group-events':
        from scripts.nvme_endurance_figures import lesson as selected
        return selected(figure)
    if figure['report_id'] == 'base-sq-associations':
        from scripts.nvme_sqa_figures import lesson as selected
        return selected(figure)
    if figure['report_id'] == 'base-read-recovery-level':
        from scripts.nvme_rrl_figures import lesson as selected
        return selected(figure)
    ...
    if figure['report_id'] == 'base-directives-streams':
        from scripts.nvme_directives_figures import lesson as selected_lesson
        return selected_lesson(figure)
