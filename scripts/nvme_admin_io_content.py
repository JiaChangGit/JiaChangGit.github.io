...
unit('command-map',BASE,'5, 5.1.1','202-204',
 ...
 [('用一台剛開始工作的 SSD 想像全貌','作業系統需要知道 SSD 能做什麼、有哪些可存取的 namespace，以及如何建立資料傳輸的佇列。這些是管理面的問題。等環境準備好，讀取某一段 LBA、寫入資料或要求持久保存，才是資料面的工作。兩種命令最後都有完成回覆，但回覆能證明什麼，要看那一種命令的定義。'),
  ...
  ('能力與當前狀態是兩次判斷','Identify 回報某命令受支援，仍不能保證每個時間點都能執行。§5.1.1 說明 Format 進行時的管理命令限制；讀 Figure 144 時先選 Format 欄，再看本篇保留的命令列和附帶條件。另一個清除作業欄屬既有專篇，本篇不展開其流程。')],
 ...
 ('Figure 143 只讀本篇保留的命令列；Figure 144 讀 Format 欄及本篇保留的列。先辨識命令集，再辨識狀態限制。',
  'Read only retained command rows in Figure 143 and the Format column for retained commands in Figure 144. Establish the command set before state-dependent restrictions.'))
...
unit('firmware',BASE,'5.2.9, 5.2.10','228-232',
 ...
 [('報告順序與執行順序可以分開','為了順著 PDF 翻頁，先在 §5.2.9 說明最終有哪些保存和啟用選擇，再往後讀 §5.2.10 的資料傳輸。口頭先交代「實際更新會先下載」，就不必為了展示流程來回翻兩節。此處只讀 firmware 的 CA 選項，Boot Partition 動作留在既有專篇。'),
 ...
 ('Figure 187–189 解釋 CA／FS、MUD 與啟用結果；Figure 190–193 解釋資料位址、數量、位移及重疊。原表的 Boot 專用動作不在本篇展開。',
  'Figures 187–189 explain CA/FS, MUD, and activation results; Figures 190–193 explain the buffer, count, offset, and overlap. Boot-specific actions are outside this report.'))
...
unit('logs',BASE,'5.2.13, 5.2.13.1.1–5.2.13.1.6, 5.2.13.1.18, 5.2.13.1.33, 5.2.13.2, 5.2.13.4','238-255,302-304,327-328,345,362',
 ...
 ('本篇保留 LID 00h、01h、02h、03h、04h、05h、12h、24h。Get Log Page 的長度、位移與事件保留是共同機制；各 Log 的範圍、單位與有效條件由其資料結構決定。',
  'This report retains LIDs 00h, 01h, 02h, 03h, 04h, 05h, 12h, and 24h. Transfer length, offset, and event retention are common mechanisms; each log defines its scope, units, and validity conditions.'),
...
unit('identify',BASE,'5.2.14.1, 5.2.14.2, 5.2.14.5','362-431',
 ...
 [('先選「問誰、問哪一種資料」','CNS=01h 讀控制器共通資料；CNS=02h 讀 active namespace ID 清單；CNS=03h 讀 namespace 的識別描述子。CNS=05h／06h 還要由 CSI 決定命令集版本的 namespace／controller 結構。先把要求寫成一句完整的問題，會比背一長串 CNS 數值容易。'),
  ...
  ('只讀資料結構，不把排除的管理流程帶回來','CNS=1Dh 的 Underlying Namespace List 描述背後的 namespace 對應，CNS=20h 回報支援的 controller state formats；本篇只教列表與欄位意義，不展開已排除的 export 或 migration 操作。UUID List、Primary／Secondary Controller 與使用者列出的其他 CNS 不讀。NVM 特定 LBA 大小和格式表會在 NVM §4.1.5 集中補上。')],
...
unit('security',BASE,'5.2.28, 5.2.29','480-482',
 ...
 [('先理解外層與內層','NVMe 在這裡提供兩個資料傳輸命令。傳入的 buffer 裡還有安全協定自己的訊息與狀態，因此要先確認外層 NVMe 命令是否完成，再按內層協定解讀結果。這份報告不把外部安全規格的全部流程加入主範圍。'),
  ...
  ('不是每一筆 Receive 前都有 Send','SECP=00h 的 Receive 用來查詢支援的安全協定，規格明確說它不對應先前的 Send。其他協定的請求與回覆如何配對、重設後資料是否仍在，都要依協定與本節條件判斷；不能只用「上一筆 Send」推論。')],
...
unit('queues',BASE,'5.3.1–5.3.4','553-558',
 ...
 [('先畫引用關係，就知道先後順序','一條 Completion Queue 可以服務多條 Submission Queues。建立 SQ 時要填入目標 CQID，所以不能先建立一條指向不存在 CQ 的 SQ。Number of Queues Feature 只完成可用數量的協商，不會替主機建立這些記憶體佇列。'),
  ...
  ('刪除時不要把記憶體太早拿走','先處理 SQ 的刪除與原先命令的結果，再刪除已沒有 SQ 引用的 CQ。Delete SQ 的完成結果與其中每一筆舊命令如何結束，要按本節與其引用的 queue lifetime 條件確認。Invalid Queue Identifier、Invalid Queue Size、Invalid Completion Queue 等錯誤各指出不同配置問題，不能都當成「資源不夠」。')],
...
unit('dataset',NVM,'3.3.3','44-48',
 ...
 [('用「不再需要這段資料」來理解解除配置','檔案系統刪掉檔案後，可以告知控制器某些 LBA 的舊內容不再需要。這是空間使用資訊，不是這篇已排除的完整資料清除功能。後續讀回內容由 namespace 能力與 deallocated／unwritten 的規則決定。'),
  ('命令數量與每段長度要分別解碼','NR 是範圍描述子的個數，採從 0 起算；每筆描述子的 LLB 是直接的 block count。SLBA 是該範圍起點，Context Attributes 描述存取模式。AD、IDR、IDW 表達要提供哪一類要求或提示，各自不是另一個長度。'),
...
unit('nvm-features',NVM,'4.1.3.1, 4.1.3.2, 4.1.3.4','64-67',
 ('NVM 功能補充：工作負載、LBA 用途與原子性','NVM features: workload, LBA usage, and atomicity'),
...
unit('format-list',NVM,'5.6','160-162',
 ('用 LBA Format List 收尾：相同欄位位置不代表同一筆格式','Finish with LBA Format List interpretation'),
