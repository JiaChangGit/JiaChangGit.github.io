"""Source-specific figure teaching tables; captions and page evidence live in the register."""
try:
...
f(1,'Base,PCIe Transport,NVM Command Set','依功能層分開閱讀：Base 給共同機制，PCIe 給本機 transport，NVM 給 logical-block 命令語意。','Read by functional layer: Base supplies shared mechanisms, PCIe the local transport, and NVM logical-block command semantics.')
...
f(4,'AWUN,AWUPF,ACWU,NAWUN,NAWUPF,NACWU,NABSN,NABSPF,NABO','先用 NSABP 選 controller 或 namespace 值，再依欄位換算 0-based 大小及 namespace 的零值繼承規則；不可把每個 raw 0 都解釋成一個 block。','Use NSABP to select controller or namespace parameters, then interpret the fields zero-based sizes and namespace zero-value inheritance. Raw zero does not universally mean one block.')
f(5,'Overlapping writes,Read result,AWUN','本圖討論正常運作下重疊寫入的可觀測結果；將 write size 與依欄位換算後 AWUN 比較，再看讀取是否落在相同 atomic unit。','This table concerns observable overlapping-write results during normal operation. Compare write size with interpret the fields AWUN and find the read within atomic units.')
...
f(13,'Get LBA Status,I/O controller,Administrative controller','Get LBA Status 86h 對 I/O controller 是 optional、對 Administrative controller 禁止。','Get LBA Status 86h is optional for an I/O controller and prohibited for an Administrative controller.')
...
f(18,'SCT,SC,LBA Out of Range,Capacity Exceeded','Generic status 80h 是 LBA Out of Range，81h 是 Capacity Exceeded；依欄位換算前保留 SCT，並分別核對 NSZE 與 NCAP／NUSE。','Generic status 80h is LBA Out of Range and 81h Capacity Exceeded. Retain SCT and check NSZE separately from NCAP/NUSE.')
...
f([32,44],'DPTR','DPTR 指向 descriptors，而不是待複製或 deallocate 的 user data；buffer 長度需由 descriptor size 與依欄位換算後 range count 計算。','DPTR addresses descriptors rather than user data to copy or deallocate. Compute buffer length from descriptor size and the range count after reading the fields.')
f([51,135],'DPTR','DPTR 是 controller 回傳資料的 host 目的位置；Read 回 data，Get LBA Status 回 descriptor list；兩者的回傳資料格式不同。','DPTR addresses the host destination: Read returns data and Get LBA Status returns a descriptor list. Their response data formats differ.')
...
f(42,'Source ranges,SDLBA,Destination offsets','依 source descriptor 順序把各段長度相加，產生連續 destination；圖中的加總是 block 數，不能直接累加 raw NLB 而漏掉每段的一。','Sum the source lengths after reading the fields in descriptor order to form the contiguous destination. The diagram sums block counts, not raw NLB values missing one block per range.')
...
f(47,'CATTR,LLB,SLBA','每筆 16 bytes：CATTR、1-based LLB、64-bit SLBA；256 筆需要 4096 bytes。不要沿用 NLB 的加一依欄位換算。','Each 16-byte entry contains CATTR, one-based LLB, and 64-bit SLBA; 256 entries require 4096 bytes. Do not apply NLB’s add-one decoding.')
...
f(114,'NEID,NLRD,RATYPE','NEID 找出 namespace，RATYPE 建議後續 Get LBA Status 的 ATYPE；NLRD=FFFFFFFFh 表示無 range list 且宜檢查全 namespace。','NEID identifies the namespace and RATYPE recommends the later Get LBA Status ATYPE. NLRD=FFFFFFFFh means no range list is available and the whole namespace should be examined.')
...
f(120,'SC,SI,NNSMAD,RLMA','先以 SC 選 scope 再解讀 SI 與巢狀 access list；本表 SI 說明仍提 NSET 且 byte 例子錯置，應以 SC 與表列 bytes7:4 找出。','Select scope with SC before interpreting SI and nested access lists. The SI prose still mentions NSET and misplaced example bytes; SC and the tabulated bytes7:4 provide the field location.')
...
f(147,'NPWA,NPWG','NPWA 指起點，NPWG 指長度 granularity；示意的 8 blocks 是依欄位換算後大小，並非 raw field=8。','NPWA governs the start and NPWG the length granularity. The illustrated eight blocks are a size after reading the fields, not raw field value eight.')
...
f(192,'NLBAF,NULBAF,Format Index','先把 raw NLBAF 加1成共同格式數，再加 NULBAF；unique attributes 區緊接在共同區之後。','Read the fields raw NLBAF by adding one, then add NULBAF. Unique-attribute formats immediately follow the common-format region.')
...
f(196,'Dword offsets,Byte ranges,LPL,Shared descriptors','描述子以相對於 log 起點的 dword offset 連接。例如 offset=299 對應 299×4=1196 bytes；共享的 descriptor 可由多條連接指向。','Descriptors connect through dword offsets from the log start. For example, offset=299 means 299×4=1196 bytes; multiple connections can reference a shared descriptor.')
...
f(189,'ECNTLID,MDTS,RAB,NCQS,NSQS,MQES,ONCS,AWUN,AWUPF','ECNTLID=0，限制值不得超過 underlying；NCQS／NSQS 先解 0-based。WZS／DSMS 需對應底層能力與所報 Commands Supported and Effects。','ECNTLID=0 and limits must not exceed underlying support. Read the fields zero-based NCQS/NSQS. WZS/DSMS must correspond to underlying support and reported Commands Supported and Effects.')
...
BASE_DETAILS = {
 ...
 116: ('READ-WRITE','SGL descriptor,Address,Length,Type', b('本機 SGL descriptor 用 address、length 與 type 描述 buffer。','A local SGL descriptor describes a buffer with address, length, and type.')),
 338: ('IDENTIFY','ONCS,CTRATT,MDTS,SANICAP', b('NVM 需要 Base Identify 的 ONCS variants、CTRATT.ELBAS／MEM、MDTS 與 SANICAP 等能力。','NVM uses Base Identify capabilities including ONCS variants, CTRATT.ELBAS/MEM, MDTS, and SANICAP.')),
 ...
 451: ('SANITIZE','SANACT,AUSE,NDAS,EMVS,PREQ', b('Subsystem Sanitize 的 CDW10 分開方法及修飾 bits；NVM §4.1.7 使用這個 Base 命令格式。','Subsystem Sanitize CDW10 separates method from modifiers. NVM §4.1.7 uses this Base command format.')),
 ...
 561: ('MIGRATION-QUEUE','MOS,MO', b('Track Send CDW10 先選 management operation，MOS 再依 operation 解讀；Log User Data Changes 用來追蹤 NVM 資料變更。','Track Send CDW10 selects a management operation before MOS is interpreted. Log User Data Changes tracks changes to NVM data.')),
 ...
 712: ('STREAMS','SWS,SGS', b('Streams Return Parameters 的 SWS／SGS 描述寫入大小與配置粒度；NVM §5.13 將 SWS 的 command-set 單位定義為 logical blocks。','SWS/SGS in Streams Return Parameters describe write size and granularity. NVM §5.13 defines the command-set unit of SWS as logical blocks.')),
}
...
def expanded_guide(figure, language):
    ...
    return dict(kind='layout', context=data['text'][language],
        kind_text=('Field relationships and a worked interpretation, redrawn for this source and revision.' if en else '依本來源與版本重整欄位關係，配合依欄位換算案例閱讀。'),
        terms=terms,
        steps=[('Find the cited page range and identify raw versus interpret the fields units.' if en else '找出引用的完整跨頁範圍，區分 raw 值與依欄位換算後單位。'),
...
for names,zh,en in [
 ...
 ('NCQS,NSQS,MQES','支援的 completion／submission queue 數與 queue entries；先解 0-based，再核對 underlying 上限。','Supported completion/submission queue counts and queue entries; interpret the fields zero-based values and check underlying limits.'),
...
def definition(term, language):
    ...
    # Each teaching table immediately above supplies the particular field relationship;
