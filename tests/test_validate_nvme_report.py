...
class NvmeReportContractTest(unittest.TestCase):
    ...
    def test_contract_has_twenty_reports_and_sixty_artifacts(self):
        contract = json.loads(
            (ROOT / ".ai/nvme-report/output-contract.json").read_text(encoding="utf-8")
        )
        artifacts = contract["artifacts"]
        self.assertEqual(len(artifacts), 66)
        self.assertEqual(sum(item["format"] == "html" for item in artifacts), 22)
        self.assertEqual(sum(item["format"] == "markdown" for item in artifacts), 44)
        self.assertEqual(len({item["report_id"] for item in artifacts}), 22)
        self.assertEqual(
            {item.get("parity_group") for item in artifacts if item["format"] == "markdown"},
            {
                ...
                "nvmcs13-bilingual", "adminio-bilingual", "streams-bilingual", "fdp-bilingual", "lockdown-bilingual", "rrl-bilingual", "sqa-bilingual", "ege-bilingual", "nwp-bilingual", "pel-bilingual",
            },
        )
