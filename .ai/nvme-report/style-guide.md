...
目前保留 19 篇，允許獨立閱讀所需的跨篇背景，同篇不反覆解釋。
...
2026-09-14：中文教學 HTML 與開 Spec 報告用途分離；學習順序不得受 PDF 順序限制。保留引用供核對，但不能用「依本節條件」「自行參考 Spec」代替必要教學。逐圖教學須說明欄位位置、值的編碼、有效條件及至少一個具體判讀；同一組共同規則只解釋一次。流程圖需放在相關段落之間，不把全部圖堆在長文末尾。中英文 post 保留全局與順向 Spec 路徑。
